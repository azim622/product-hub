...
const Navbar = () => {
    const isUser = false; // change to true when login
    const profileImage = isUser 
        ? "https://i.ibb.co.com/MkWNmwbR/470203565-2390972847913826-3804411578746235717-n.jpg"
        : "https://i.ibb.co.com/MkWNmwbR/470203565-2390972847913826-3804411578746235717-n.jpg"; // default profile image
                        ...
                        <Menu as="div" className="relative ml-3">
                            <div>
                                <MenuButton className="relative flex rounded-full bg-gray-800 text-sm focus:ring-2 focus:ring-white focus:ring-offset-2 focus:ring-offset-gray-800 focus:outline-hidden">
                                    ...
                                    <img alt="Profile" src={profileImage} className="size-8 rounded-full object-cover" />
                                </MenuButton>
                            </div>
                            ...
                            <MenuItems transition className="absolute right-0 z-10 mt-2 w-48 origin-top-right rounded-md bg-white shadow-lg ring-1 ring-black/5 transition focus:outline-hidden data-closed:scale-95 data-closed:transform data-closed:opacity-0 data-enter:duration-100 data-enter:ease-out data-leave:duration-75 data-leave:ease-in">
                                {
                                    isUser ? (
                                        <>
                                            <MenuItem>
                                                <Link to="/profile" className="block px-4 py-2 rounded-t-md text-sm text-gray-700 hover:bg-gray-100">Your Profile</Link>
                                            </MenuItem>
                                            <MenuItem>
                                                <Link to="/settings" className="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100">Settings</Link>
                                            </MenuItem>
                                            <MenuItem>
                                                <Link to="/sign-out" className="block px-4 py-2 rounded-b-md text-sm text-gray-700 hover:bg-gray-100">Sign out</Link>
                                            </MenuItem>
                                        </>
                                    ) : (
                                        <MenuItem>
                                            <Link to="/sign-in" className="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100">Sign In</Link>
                                        </MenuItem>
                                    )
                                }
                            </MenuItems>
